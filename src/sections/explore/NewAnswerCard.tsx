...
export default function NewAnswerCard({
  question, className, setOpen, status, token, isUserQuestionPage
}: Props) {
  const srcSet =
  ...
  const [requestType, setRequestType] = useState<'redeem' | 'answer' | 'listen'>('answer');
  ...
  const questionMint = async () => {
    ...
    const tx = await echoinn.connect(provider.getSigner()).questionMint(account, kolAddress, question, { value: award });
    await tx.wait();
  }
  ...
  const handleListenQuestion = async () => {
    ...
    try {
      // contract question mint.
      const tx = await echoinn.connect(provider.getSigner()).subscribe(tokenId,{value:subscribeFee});
  ...
  // fetch the question price from the chain
  ...
  return (
    <Box sx={{ mt: 2, mb: 2, mx: 3, textAlign: "left" }}>
      ...
      <Stack>
        ...
        >
          {findQuestionUserName(question.ownerAddress)}
        </Typography>
        <Typography variant="body1" sx={{ color: 'text.secondary' }}>
          Total profit:
        </Typography>
      ...
      {/* {<Stack direction="row" sx={{ mt: 4, justifyContent: 'space-between' }}>
        <Button sx={{ mt: 2, fontSize: '24px', py: 4, minWidth: 120, mx: 'auto' }} variant="contained" size="large" onClick={handleAnswerQuestion}>Answer it</Button>
      ...
      {isUserQuestionPage && status === "UNFINISHED" && <Stack direction="row" sx={{ mt: 4, justifyContent: 'space-between' }}>
        <Button sx={{ my: 2, fontSize: '24px', py: 4, minWidth: 120, mx: 'auto' }} variant="contained" size="large" onClick={handleRedeemQuestion}>Redeem Question</Button>
      </Stack>}
      {/* comments */}
      {status === 'purchased' && (
      ...
      {chainRequeststatus && <MessageBox type={requestType} status={chainRequeststatus} message={message} />}
    </Box>
  );
}
