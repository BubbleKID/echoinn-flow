...
import AnswerCard from '../../../explore/AnswerCard';
import NewAnswerCard from '../../../explore/NewAnswerCard';
...
export default function KOLConsole({ friends, findFriends, onFindFriends }: Props) {
  ...
  return (
    <Box sx={{ mt: 5 }}>
      ...
      {!loading &&<Grid container spacing={3}>
        {
          data && data.getAllQuestions.map((question: any) => (question.kolAddress === account && question.status === "FINISHED") && (
      ...
      {!loading &&<Grid container spacing={3}>
        {
          data && data.getAllQuestions.map((question: any) => question.status === "UNFINISHED" && (
            <Grid key={question.id} item xs={12} md={3}>
              <NewQuestionCard openDetailModel={() => openDetailModel(question)} isKOLConsole questionData={question} status="editor"/>
            </Grid>
          ))
        }
      </Grid>}
      ...
      >
        <Box sx={style}>
          {currentQuestion && currentQuestion.status === "UNFINISHED" && (<NewProfileAnswerCard setOpen={setOpen} question={currentQuestion} status="UNFINISHED" token={token}/>)}
          {currentQuestion && currentQuestion.status === "FINISHED" && (<NewAnswerCard isUserQuestionPage={false} setOpen={setOpen} question={currentQuestion} status="FINISHED" token={token}/>)}
