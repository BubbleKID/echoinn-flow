...
export function getProducts() {
  return async () => {
    ...
    try {
      const response: { data: { products: Product[] } } = await axios.get('https://minimal-assets-api-dev.vercel.app/api/products');
...
export function getProduct(name: string) {
  return async () => {
    dispatch(slice.actions.startLoading());
    try {
      const response: { data: { product: Product } } = await axios.get('https://minimal-assets-api-dev.vercel.app/api/products/product',{
        params: { name },
      });
